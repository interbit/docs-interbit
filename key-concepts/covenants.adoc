...
to blockchains via link:chain_joins.adoc[chain joins]. And, since
Interbit applications are built on link:https://reactjs.org/[React], they
can share components and functionality. This provides a high degree of
modularity and streamlines development, but more importantly, Interbit
applications can inter-operate even when they are hosted by distinct
parties.
